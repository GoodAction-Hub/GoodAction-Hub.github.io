...
const SPARK_API_PASSWORD = (
  process.env.SPARK_API_PASSWORD ||
  process.env.IFLYTEK_SPARK_API_PASSWORD ||
  ''
).trim()
...
async function chatSparkWs({
  ...
  if (!APP_ID || !API_KEY || !API_SECRET) {
    throw new Error(
      '星火API环境变量未配置：IFLYTEK_APP_ID、IFLYTEK_API_KEY、IFLYTEK_API_SECRET',
    )
  }
...
export async function chatSparkX1Http({
  ...
  if (!apiKeyForHttp) {
    throw new Error(
      '星火X1 HTTP环境变量未配置：SPARK_API_PASSWORD 或 IFLYTEK_API_KEY/SECRET',
    )
  }
  ...
  if (!resp.ok) {
    ...
    const err = new Error(
      `X1 HTTP调用失败: ${resp.status} ${errText}`,
    ) as HttpErrorLike
