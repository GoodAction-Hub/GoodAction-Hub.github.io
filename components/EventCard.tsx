...
export const EventCard: FC<EventCardProps> = observer(
  ({ item, event, displayTimezone }) => {
    ...
    const ended = isEventEnded(event, currentTimezone);
    ...
    const upcomingDeadlines = event.timeline
      ...
      // 转换到显示时区进行比较
      .filter((t) => t.date.setZone(currentTimezone) > now)
      .sort((a, b) => a.date.toMillis() - b.date.toMillis());

    const nextDeadline = upcomingDeadlines[0];

    // 转换时区为UTC偏移格式
    const displayTimezoneUTC = formatTimezoneToUTC(currentTimezone);
    ...
    useEffect(() => {
      if (showScrollHint && scrollViewportRef.current && activeDotRef.current) {
        ...
        const targetScrollLeft =
          activeLeft - viewportWidth / 2 + activeWidth / 2;
